...
export default function BriefingBanner({ events, userEmail, darkMode }) { // 💡 darkMode 수신 구멍 확보
  const [now, setNow] = useState(new Date())
  
  const [weather, setWeather] = useState({
    temp: '24', rainChance: '0', desc: '맑음', location: '내 위치', windSpeed: '0.0', humidity: '0', precip: '0.0', loading: true
  })
    ...
    const fetchWeatherLive = () => {
      fetch('https://wttr.in/?format=j1')
        ...
        .then(data => {
          ...
          setWeather({
            temp: current?.temp_C || '24', rainChance: maxRainChance, desc: koreanDesc, location: koreanLocation, windSpeed: windMs, humidity: current?.humidity || '0', precip: current?.precipMM || '0.0', loading: false
          })
        })
        .catch(err => {
  ...
  useEffect(() => {
    ...
    let scheduleText = "";
    if (todayEvents.length === 0) { scheduleText = "오늘 등록된 일정이 없습니다."; } 
    else if (activeOrUpcomingEvents.length === 0) { scheduleText = "오늘 모든 일정을 마쳤습니다."; } 
    else {
    ...
    if (rainNum >= 50) { weatherAlertText = " 현재 강수 확률이 높으니 외출 시 우산을 꼭 챙기세요!"; } 
    else if (windNum >= 4.5) { weatherAlertText = ` 현재 바람이 ${weather.windSpeed}m/s로 강하게 부니 안전에 유의하세요.`; } 
    else { weatherAlertText = " 쾌적한 날씨 속에서 기분 좋은 하루 보내세요!"; }

    setDynamicBriefing(`${scheduleText}${weatherAlertText}`);
  }, [now, events, weather.rainChance, weather.windSpeed, weather.desc]) 
  ...
  return (
    // 💡 [직각 유지 + 흑백 반전 완벽 구현]
    <div className={`border p-6 rounded-none shadow-none flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative min-h-[140px] transition-colors duration-300 ${
      darkMode ? 'border-neutral-800 bg-neutral-900/60' : 'border-neutral-200 bg-neutral-50/10'
    }`}>
      ...
      <div className="flex-1 w-full space-y-4 pr-4">
        <div className="space-y-2">
          ...
          {/* 💡 다크모드 시 뚜렷한 흰색 반전 */}
          <h2 className={`text-sm leading-relaxed font-normal ${darkMode ? 'text-white' : 'text-neutral-800'}`}>
            {dynamicBriefing}
          </h2>
        </div>
        ...
        <div className="flex items-baseline gap-1.5 text-xs select-none pt-1">
          ...
          <span className={`italic leading-relaxed font-normal tracking-tight ${darkMode ? 'text-neutral-400' : 'text-neutral-500/90'}`}>
            {todayFortune}
          </span>
        </div>
      </div>

      {/* 💡 [우측 분리선 하얀색 처리] 다크 모드 시 수직 경계선을 bg-white/20 으로 매핑 */}
      <div className={`shrink-0 w-full md:w-auto md:self-stretch flex flex-col items-end justify-center pl-8 border-t md:border-t-0 md:border-l font-sans text-right select-none space-y-1 transition-colors ${
